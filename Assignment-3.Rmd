...
```{r}
# Chromosome name is in V3 - RNAME
unique(sam$V3)



# Count reads aligning to tdTomato
num_tdTomato_reads <- sum(sam$V3 == "tdTomato")
paste("Number of reads aligning to tdTomato:", num_tdTomato_reads)
```
63 reads is a sign of low-level expression. There is a few steps that are needed for a cell to be fluorescent. If the tomato protein does not mature properly or if the expression level is not strong enough, it will be difficult to observe fluorescence under the microscope. Assuming that the cells are fluorescent, tdtomato might be used as a way to ensure the transduction or transfection of a given population was done properly. It is a non invasive and non-destructive way of assesing the expression of specific genes. For example, if a gene of interest was added to the genome of a mouse, attached to a tdtomato gene, the presence of tdtomato indicated the successful expression of the gene of interest as well.
...
# PART 1: CALCULATE VAF VALUES 
# Parse the genotype field to extract allele depths and calculate VAF
VAF_values <- c()  # Initialize empty vector to store VAF for each variant

for (i in 1:nrow(variants)) {
  format_field <- as.character(variants$FORMAT[i])
  genotype_field <- as.character(variants[i, 10])  
  format_fields <- strsplit(format_field, ":")[[1]]
  genotype_values <- strsplit(genotype_field, ":")[[1]]

  # Find which position contains the AD (Allele Depth) field
  AD_index <- which(format_fields == "AD")

  # If AD field exists, calculate VAF
  if (length(AD_index) > 0) {
    allele_depths <- genotype_values[AD_index]
    depths <- as.numeric(strsplit(allele_depths, ",")[[1]])

    # Calculate VAF = alt_count / (ref_count + alt_count)
    if (length(depths) >= 2 && (depths[1] + depths[2]) > 0) {
      VAF <- depths[2] / (depths[1] + depths[2])
      VAF_values <- c(VAF_values, VAF)
    } else {
      
      # If depths are invalid, store NA
      VAF_values <- c(VAF_values, NA)
    }
  } else {
    
    # No AD field found, store NA
    VAF_values <- c(VAF_values, NA)
  }
}

# Add VAF as a new column to the variants dataframe
variants$VAF <- VAF_values
